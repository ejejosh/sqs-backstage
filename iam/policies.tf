data "aws_iam_policy_document" "tiko_nudge" {
  statement {
    ...
    resources = [
      ...
      "arn:aws:sqs:${data.aws_region.current.name}:${data.aws_caller_identity.current.account_id}:${var.environment}-*-acl-give-tiko-miles",data.terraform_remote_state.sqs_sns.outputs.queue_arns["${var.environment}-test"],
  ...
  statement {
    ...
    resources = [
      ...
      data.terraform_remote_state.sqs_sns.outputs.queue_arns["${var.environment}-acl-give-tiko-miles-responses"]data.terraform_remote_state.sqs_sns.outputs.queue_arns["${var.environment}-test"],
